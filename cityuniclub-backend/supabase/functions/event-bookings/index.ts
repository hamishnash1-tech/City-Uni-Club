  ...
  try {
    ...
    if (resendKey) {
      ...
      const priceText = event.price_per_person
        ...
        : 'TBA'
      const bookerName = member ? [member.first_name, member.middle_name, member.last_name].filter(Boolean).join(' ') : guest_name
      ...
      // Secretary notification for all bookings
      await sendEmail(resendKey, {
        from: FROM_EMAIL,
        to: [CLUB_EMAIL],
        subject: `New event booking — ${escapeHtml(event.title)}`,
        html: `
          <html><body style="font-family:Arial,sans-serif;line-height:1.6;color:#333;max-width:600px;">
            <h2 style="color:#002147;">New Event Booking</h2>
            <div style="background:#f5f5f5;padding:15px;border-left:4px solid #002147;margin:20px 0;">
              <p style="margin:0;"><strong>Event:</strong> ${escapeHtml(event.title)}<br>
              <strong>Date:</strong> ${formattedDate}<br>
              <strong>Guests:</strong> ${guest_count}<br>
              <strong>Total:</strong> ${priceText}</p>
            </div>
